function varargout =Calculation_of_Stress_Threshold_English(varargin)
% CALCULATION_OF_STRESS_THRESHOLD_ENGLISH MATLAB code for Calculation_of_Stress_Threshold_English.fig

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Calculation_of_Stress_Threshold_English_OpeningFcn, ...
                   'gui_OutputFcn',  @Calculation_of_Stress_Threshold_English_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Calculation_of_Stress_Threshold_English_OpeningFcn(hObject, eventdata, handles, varargin)
...
clc
set(gcf,'NumberTitle','off','name','Calulation of Stress threshold of Pipeline with Flaw')  
set(handles.radiobutton19,'value',1);
set(handles.radiobutton1,'value',1);
function varargout = Calculation_of_Stress_Threshold_English_OutputFcn(hObject, eventdata, handles) 
...
if  isnan(shuzi)
    ferr = warndlg('Input parameters must be numbers', 'Input error');
    set(handles.edit2,'string',[]);
    return;
elseif shuzi<0
    ferr = warndlg('Input parameters must be nonnegative', 'Input error');
    set(handles.edit2,'string',[]);
    return;
  
elseif (shuzi/(0.5*shuzi8-0.5*shuzi)>0.1)||(shuzi8*0.5<shuzi)
    ferr = warndlg('The procedure applies to B/R≤0.1', 'Input error')
    set(handles.edit2,'string',[]);
    return;   
end
...
if nw27==1
      if shuzi>3.14159*shuzi8
       ferr = warndlg('The length of circumferential crack should be less than that of the periphery of the pipeline.', 'Input error');
...
elseif shuzi<0
    ...
    return;
elseif shuzi>=shuzi2
    ferr = warndlg('Crack depth size must be less than thickness', 'Input error');
    set(handles.edit4,'string',[]);
    return;
...
elseif shuzi<0
    ...
    return;
elseif shuzi>=shuzi2
    ferr = warndlg('Size P1 must be less than thickness', 'Input error');
    set(handles.edit5,'string',[]);
    return;
...
elseif shuzi<0
 ...
 elseif (shuzi+shuzi4+shuzi5>shuzi2)||(shuzi+shuzi4+shuzi5<shuzi2)
    ferr = warndlg('p1+p2+h Must be equal to thickness', 'Input error');
    set(handles.edit6,'string',[]);
    return;
end
...
%%%材料断裂韧度与按钮要一致
if (dc>0)&&(nw19==0)
    ferr = warndlg('Fracture toughness is input butthe radio button does not match the edit box ', 'Input error');    
    return  
   elseif  (di>0)&&(nw20==0)
    ferr = warndlg('Fracture toughness is input butthe radio button does not match the edit box ', 'Input error');    
    return 
...
if nw13==1  %%满足条件则输入的为表面裂纹
     result_yuan='Surface crack';
    if h>0.7*B %规则化为穿透裂纹
        a=0.5*(L+2*h);
        c=0;               %%%%%未校验
        result='Penetrated crack';
        type= 3 ;
      ...
      set(handles.text52,'visible','off');
        ...
        if h<0.5*L %规则化为椭圆表面裂纹
            a=h;
            c=0.5*L;
            result='Semi-ellipticl surface crack  a= ';
               type=  1;
        ...
        set(handles.text52,'visible','on');
                ...
                a=h;
                c=h;
                result='Semicircle surface crack  a= ';
                   type=  1;
        set(handles.text49,'visible','on');
        set(handles.text50,'visible','on');
        set(handles.text52,'visible','on');
...
fprintf('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n')
fprintf('&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&\n')
fprintf('Primary cracks before regularization are')
fprintf('%s',result_yuan)
fprintf('\t wall thickness of Pipe  B=')
fprintf('%d',B)
fprintf('\t h=The depth of crack h=')
fprintf('%d',h)
fprintf('\t The length of crack L=')
fprintf('%d\n',L)
 
if type==1
fprintf('After regularization, the crack is')
fprintf('%s',result)
fprintf('%d\t',a)
fprintf('2c=')
fprintf('%d\n',2*c)
...
fprintf('After regularization, the crack is：')
fprintf('%d\n',aa) 
fprintf('The temperature difference between inner and outer walls is ：')
fprintf('%d\n',dt) 
...
if nw1==1    %进入简化评定程序
    cla;
    fprintf('The following are the basic parameters for simplified  evaluation\n')
 axis([0 1 0 1]);
    %以下为载荷比参量  Lrmax的计算
if ((os+ob)/(2*os))<1.2
    Lrmax=(os+ob)/(2*os);
...
%%以下为δ及sqrt(δr)的计算
 ...
 pause(0.0001);
     if (Sr>0.8)||(sdr>0.7)
        set(handles.text62,'string','Simplified Evaluation');
        set(handles.text64,'string',num2str((o-1*10^6)/1000000));
     end
     if rem(i,20)==0
         fprintf('σ1=')
...
if nw2==1     %进入常规评定程序
    cla;
    fprintf('The following are the basic parameters of conventional  assessment\n')
        if  nw14==1      %失效后果为一般
   ...
   hold on;
 Lr=0;o=0;;oo=0;KrA=0;KrB=0;yLrMAX=1;i=-1;Krmax_Lr=1;
   %%%%%%%%%%%%%%%%%%%%%%%%%%开始循环
 ...
 switch type
     ...
     if  nw25==1          %认为纵向穿透裂纹的意思是裂纹是轴向的      已经问过
         if B/Ri<=0.1
           ...
           ferr = warndlg('This procedure applies only toB/R<0.1', 'error');
 ...
 %%以下关于塑性修正系数的计算
   ...
   pause(0.0001)
     ...
     if (Lr>1.25)||(KrA>Krmax_Lr)||(KrB>Krmax_Lr)
        set(handles.text62,'string','Routine Evaluation');
...
clear all
winopen ('C:\含裂纹型缺陷管道的应力阈值计算软件\软件运行附属材料\ywsysm.jpg')
...
delete(gcf)
Calculation_of_Stress_Threshold;
